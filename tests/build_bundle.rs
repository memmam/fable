//! End-to-end tests for `fable build`: a stapled binary must run its embedded
//! program and produce byte-for-byte the same output as running that program
//! from source with `fable demos/<name>/main.fable`. Covers the paths most
//! likely to break under extraction: imported modules, a bundled data file
//! read at runtime, and worker isolates spawned from a separate `.fable`.
...
/// `fable demos/<demo>/main.fable`, from the repo root — the canonical run a
/// stapled binary must reproduce.
fn run_from_source(demo: &str) -> Vec<u8> {
    let out = Command::new(fable())
        .current_dir(repo_root())
        .arg(format!("demos/{demo}/main.fable"))
        .output()
...
/// Worker isolates spawned from a separate `.fable` file, resolved against
